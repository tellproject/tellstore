...
public:
     ...
     */
    inline uint32_t getBaseKnowledge(const Table *table, const char *&basePtr, uint32_t &capacity) const {
...
#define COMPUTE_BASE_KNOWLEDGE(table) const char *basePtr; \
    uint32_t capacity; \
    uint32_t index; \
    index = getBaseKnowledge(table, basePtr, capacity);
    ...
    inline const uint32_t getRecordCount(const char *basePtr) const {
        return *(reinterpret_cast<uint32_t*>(basePtr + 4));
    }
    ...
    inline const char *getNewestPtrAt(const uint32_t index, const char * basePtr, const uint32_t capacity) const {
    ...
    inline char *getNullBitMapAt(const uint32_t index, const char * basePtr, const uint32_t capacity, const size_t nullBitMapSize) const {
        return basePtr + 8 + (capacity*24) + (index*nullBitMapSize);
    }

    inline const int32_t *getVarsizedLenghtAt(const uint32_t index, const char * basePtr, const uint32_t capacity, const size_t nullBitMapSize) const {
        return reinterpret_cast<const int32_t*>(basePtr + 8 + (capacity*(24 + nullBitMapSize)) + (index*4));
    }

    inline char *getColumnNAt(const Table *table, const uint32_t N, const uint32_t index, const char * basePtr, const uint32_t capacity, const size_t nullBitMapSize) const {
